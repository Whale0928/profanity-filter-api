...
public class RedisConfig {
  ...
  public RedisConnectionFactory redisConnectionFactory() {
    if (redisProperties.isSingleMode()) {
      ...
      // 패스워드 설정
      if (redisProperties.main().password() != null
          && !redisProperties.main().password().isBlank()) {
      ...
      LettuceClientConfiguration lettuceClient =
      ...
      RedisStaticMasterReplicaConfiguration config =
          new RedisStaticMasterReplicaConfiguration(
              redisProperties.host(), redisProperties.main().port());
      ...
      // 복제 노드들 추가
      redisProperties.sub().port().forEach(port -> config.addNode(redisProperties.host(), port));
