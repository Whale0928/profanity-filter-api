...
public class InmemoryClientsRepository implements ClientsRepository {
  ...
  public boolean existsByEmail(String email) {
    return store.values().stream().anyMatch(client -> client.getEmail().equals(email));
  }

  @Override
  public boolean existsByApiKey(String apiKey) {
    return store.values().stream().anyMatch(client -> client.getApiKey().equals(apiKey));
  }

  @Override
  public Optional<Clients> findByEmail(String email) {
    return store.values().stream().filter(client -> client.getEmail().equals(email)).findFirst();
  }

  @Override
  public Optional<Clients> findByApiKey(String apiKey) {
    return store.values().stream().filter(client -> client.getApiKey().equals(apiKey)).findFirst();
  }
