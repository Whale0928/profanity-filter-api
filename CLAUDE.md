...
- **운영 도메인**: `api.kr-filter.com`(Cloudflare proxied, 신규·권장) / `api.profanity.kr-filter.com`(DNS-only, 레거시) / 문서: `whale0928.github.io/profanity-filter-api`
...
- **클라이언트 IP 보존**: `api.kr-filter.com`은 Cloudflare proxied(`CF-Connecting-IP` 전달)라 실제 IP가 `records`에 기록됨. `api.profanity.kr-filter.com`은 DNS-only(2-level 서브도메인이 Cloudflare 무료 Universal SSL 미지원)라 Cloudflare를 거치지 않아 klipper-lb L4 SNAT로 내부망 IP만 기록됨 → 신규 도메인으로 이전 유도 필요
