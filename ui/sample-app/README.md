...
콘솔 데모는 아직 라우트에 연결하지 않고 `src/demo/ConsoleDemo.tsx`에 보관합니다.
...
- `src/demo/ConsoleDemo.tsx`: API Key 발급·필터 테스트 화면입니다. 현재 공개 라우트에는 연결하지 않았습니다.

### TypeScript

JavaScript에 타입을 추가한 언어입니다.
문서 화면은 OpenAPI 문서를 읽기 위한 타입을 `src/docs/types.ts`에 두고, 보관 중인 콘솔 데모는 `FilterMode`, `FilterResponse`, `ClientRegisterRequest` 타입으로 실제 API 요청 흐름을 표현합니다.
...
현재 공개 문서 화면은 API 서버의 OpenAPI JSON을 직접 읽고, 보관 중인 콘솔 데모는 브라우저 `fetch`로 실제 API를 호출합니다.
요청 캐싱, 재시도, 오류 상태 공유가 필요해지는 시점에 TanStack Query를 붙이면 됩니다.
...
`src/demo/ConsoleDemo.tsx`는 아래 실제 API를 호출합니다.
