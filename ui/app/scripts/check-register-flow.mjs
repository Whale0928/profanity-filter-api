...
assert.match(app, /currentRoute\.render/, "App must render selected routes through the route registry.");
assert.match(routes, /id:\s*"register"[\s\S]*<RegisterPage\s+onNavigate=\{navigate\}\s*\/>/, "Route registry must render the register page for the register route.");
