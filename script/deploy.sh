...
if [ "$INACTIVE_ENV" = "green" ]; then
    docker rm profanity-filter-api-green-1 || true
    docker rm profanity-filter-api-green-2 || true
    ...
    docker rm profanity-filter-api-blue-1 || true
    docker rm profanity-filter-api-blue-2 || true
