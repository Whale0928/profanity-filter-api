...
class WordManagementControllerTest extends AbstractRestDocs {
  ...
  void step_1() throws Exception {
    ...
    when(wordManagement.requestNewWord(
            currentUserId, request.word(), request.reason(), request.severity().name()))
        .thenReturn(response);

    mockMvc
        .perform(
            post("/api/v1/word/request")
        ...
        .andDo(
            document(
                "api/word/request",
                requestFields(
                    fieldWithPath("word").description("신규 등록할 비속어"),
                    fieldWithPath("reason").description("신규 등록할 비속어의 이유"),
                    fieldWithPath("severity").description("신규 등록할 비속어의 심각도"),
                    fieldWithPath("type").description("요청 타입 (ADD, REMOVE, MODIFY)")),
                responseFields(
