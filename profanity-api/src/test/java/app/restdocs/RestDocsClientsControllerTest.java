...
class RestDocsClientsControllerTest extends AbstractRestDocs {
    ...
    mockMvc
        ...
        .andDo(
            document(
                ...
                requestFields(
                    fieldWithPath("name").description("이름 또는 조직명").type(JsonFieldType.STRING),
                    fieldWithPath("email").description("이메일").type(JsonFieldType.STRING),
                    fieldWithPath("issuerInfo").description("발급자 정보").type(JsonFieldType.STRING),
                    fieldWithPath("note").description("메모").type(JsonFieldType.STRING)),
                responseFields(
                    fieldWithPath("status").type(JsonFieldType.OBJECT).description("응답 상태 정보"),
                    ...
                    fieldWithPath("data.note").type(JsonFieldType.STRING).description("등록된 메모"))));
  }
  ...
  void step_2() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/clients").header("x-api-key", key))
        .andExpect(status().isOk())
        .andDo(
            document(
                "api/clients/info",
                requestHeaders(headerWithName("x-api-key").description("클라이언트 API 키")),
                responseFields(
  ...
  void step_3() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/clients/send-email").param("email", email))
        .andExpect(status().isOk())
        .andDo(
            document(
                "api/clients/send-email",
                queryParameters(parameterWithName("email").description("이메일 주소")),
                responseFields(
  ...
  void step_5() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/v1/clients").header("x-api-key", apiKey))
        .andExpect(status().isOk())
        .andDo(
            document(
                "api/clients/discard",
  ...
  void step_6() throws Exception {
    final String apiKey = apiKeyGenerator.generateApiKey();
    var request =
        new ClientUpdateRequest("도메인(https://updated.com), 연락처(010-9999-9999)", "업데이트된 메모입니다.");
    var response =
        ClientMetadata.builder().issuerInfo(request.issuerInfo()).note(request.note()).build();
  ...
  void step_7() throws Exception {
    ...
    mockMvc
        .perform(post("/api/v1/clients/reissue").header("x-api-key", currentApiKey))
        .andExpect(status().isOk())
        .andDo(
            document(
                "api/clients/reissue",
                requestHeaders(headerWithName("x-api-key").description("현재 API 키")),
                responseFields(
                    fieldWithPath("status").ignored(),
                    fieldWithPath("status.code").ignored(),
                    fieldWithPath("status.message").ignored(),
