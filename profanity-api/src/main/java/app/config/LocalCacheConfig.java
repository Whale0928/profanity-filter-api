...
public class LocalCacheConfig {
  ...
  public CacheManager cacheManager() {
    List<CaffeineCache> caches =
        Arrays.stream(LocalCacheType.values()).map(this::createFilterCache).toList();
  ...
  public CaffeineCache createFilterCache(LocalCacheType cacheType) {
    ...
    caffeineBuilder.expireAfterWrite(cacheType.getSecsToExpireAfterWrite(), TimeUnit.SECONDS);

    // 최대 캐시 항목 수 설정
    caffeineBuilder
        .maximumWeight(cacheType.getEntryMaxSize())
    ...
    return new CaffeineCache(cacheType.getCacheName(), cache);
  }
}
